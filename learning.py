...
def tenFoldCrossValidation(neighbor_num=3, precisionRecallFileName='precisionRecallFile.csv',trueFalseTableFileName='trueFalseTable.csv'):
    ...
    for i in range(0,10):
        ...
        falsePos=collections.defaultdict(float);falseNeg=collections.defaultdict(float);
        ...
        sumTruePos=sum(truePos.values()); sumFalsePos=sum(falsePos.values()); sumFalseNeg=sum(falseNeg.values())
        precision+=(sumTruePos/(sumTruePos+sumFalsePos))
        recall+=(sumTruePos/(sumTruePos+sumFalseNeg))
        with open(trueFalseTableFileName,'a') as trueFalseTableFile:
            for key in truePos.keys():
                trueFalseTableFile.write('trial '+str(i)+','+str(key)+','+str(truePos[key])+','+str(falsePos[key])+','+str(falseNeg[key])+'\n')
